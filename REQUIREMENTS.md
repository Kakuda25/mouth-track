...
- **詳細**:
  - リアルタイムでの顔認識（30fps以上）
  - 口周辺のランドマーク検出（6点：左端、右端、上唇中央外側、下唇中央外側、上唇中央内側、下唇中央内側）
  - 顔が検出できない場合の警告表示
...
- **詳細**:
  - **開き具合（openness）**: 上下の距離（上唇中央外側と下唇中央外側の距離）
  - **幅（width）**: 左右の距離（左端と右端の距離）
  - **面積（area）**: 口の面積の推定値
...
#### F002-2: 母音分類
- **概要**: 抽出した特徴量から5つの母音（あ、い、う、え、お）を判別
- **詳細**:
  - **「あ」**: 開き具合が大きい（閾値: 0.6以上）、アスペクト比が小さい（0.8以下）
  - **「い」**: 幅が大きい（閾値: 0.7以上）、開き具合が小さい（0.3以下）、アスペクト比が大きい（1.5以上）
  - **「う」**: 開き具合が小さい（0.2以下）、幅が小さい（0.4以下）、面積が小さい（すぼめている）
  - **「え」**: 開き具合が中程度（0.3 〜 0.5）、幅が大きい（0.6以上）、アスペクト比が大きい（1.2以上）
  - **「お」**: 開き具合が中程度（0.4 〜 0.6）、幅が中程度（0.5 〜 0.7）、アスペクト比が中程度（0.9 〜 1.3、丸い）
  - 判別結果の信頼度スコア（0.0 〜 1.0）
  - 複数の母音候補を返す（上位3つなど）
...
- **詳細**:
  - 各母音の判別閾値を設定可能
  - キャリブレーション機能（ユーザーが各母音を発音して基準値を設定）
  - 設定の保存・読み込み
...
#### 特徴量の正規化
各ユーザーの口のサイズが異なるため、相対的な値を使用：
- **開き具合**: ユーザーの最大開き具合を1.0として正規化
- **幅**: ユーザーの最大幅を1.0として正規化
- **アスペクト比**: 幅/開き具合（絶対値）

#### 判別ルール
```javascript
// 疑似コード
function classifyVowel(metrics, userBaseline) {
  const { openness, width, aspectRatio, area } = metrics;
  
  // 正規化
  const normOpenness = openness / userBaseline.maxOpenness;
  const normWidth = width / userBaseline.maxWidth;
  
  // 各母音のスコア計算
  const scores = {
    'あ': calculateScoreA(normOpenness, aspectRatio),
    'い': calculateScoreI(normWidth, normOpenness, aspectRatio),
    'う': calculateScoreU(normOpenness, normWidth, area),
    'え': calculateScoreE(normOpenness, normWidth, aspectRatio),
    'お': calculateScoreO(normOpenness, normWidth, aspectRatio)
  };
...
| 母音 | 開き具合 | 幅 | アスペクト比 | 面積 |
|------|---------|-----|------------|------|
| あ   | 0.6-1.0 | 0.5-0.8 | 0.5-0.8 | 大   |
| い   | 0.0-0.3 | 0.7-1.0 | 1.5-3.0 | 中   |
| う   | 0.0-0.2 | 0.3-0.5 | 1.0-2.0 | 小   |
| え   | 0.3-0.5 | 0.6-0.9 | 1.2-2.0 | 中   |
| お   | 0.4-0.6 | 0.5-0.7 | 0.9-1.3 | 中   |
...
#### 初期段階
- 閾値ベース分類で実装
- ユーザーキャリブレーションで個人差に対応

#### 将来的な拡張
- 機械学習モデルを追加
- ユーザーが選択可能（閾値ベース or MLベース）
